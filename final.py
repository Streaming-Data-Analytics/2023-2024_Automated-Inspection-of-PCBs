...
if __name__ == "__main__":
    ...
    video_info = sv.VideoInfo.from_video_path(video_path="E.mp4")
    ...
    frame_generator = sv.get_video_frames_generator(source_path="E.mp4")

    # uso una hash map + deque
    coordinates = defaultdict(lambda: deque(maxlen=video_info.fps))

    with sv.VideoSink("E_output.mp4", video_info) as sink:
...
so.Plot(err).add(so.Bar(), so.Hist())
